...
1. Determine mode:If the first non-blank line is true (case insensitive), you must activate STRICT mode. In this mode, the user answer must be 100% identical in wording, punctuation, structure, and meaning to the stored answer. You must not accept paraphrased, summarized, or semantically similar responses. Any mismatch, no matter how small, must be graded as incorrect. You are forbidden from applying interpretation or flexibility.
2. Match answers:
