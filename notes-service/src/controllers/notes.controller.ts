...
const sendToExtractTextService = async (fileBuffer: Buffer, filename: string) => {
    const formData = new FormData();
    formData.append("file", fileBuffer, filename);
    const response = await axios.post<ExtractTextResponse>(
        "http://extracttext-service.intelligtest-namespace.svc.cluster.local:5000/extract",
        formData,
...
export const uploadNotes = async (req: Request, res: Response) => {
    ...
    if (!file || !owner || !section) {
        res.status(400).send({ error: "Missing data" });
        return;
    }
    try {
        const testVerify = await axios.get(
            `http://test-service.intelligtest-namespace.svc.cluster.local:4003/api/test/${section}/test`,
            {
    ...
    try {
        ...
        const response = await sendToExtractTextService(file.buffer, file.originalname);
        ...
        await newNote.save();
        await axios.put(
            `http://section-service.intelligtest-namespace.svc.cluster.local:4000/api/sections/${section}/update-section-note`,
            { note_id: newNote.id },
            {
                headers: {
                    "x-user-id": owner,
...
export const updateNote = async (req: Request, res: Response) => {
    ...
    const section = req.params.sectionId;
    try {
        const testVerify = await axios.get(`http://test-service.intelligtest-namespace.svc.cluster.local:4003/api/test/${section}/test`, {
            headers: { "x-user-id": owner, Authorization: owner },
            timeout: 5000
        });
        if (testVerify.status === 200) {
            await axios.delete(`http://test-service.intelligtest-namespace.svc.cluster.local:4003/api/test/${section}/test`, {
                headers: { "x-user-id": owner, Authorization: owner },
                timeout: 5000
            });
        }
